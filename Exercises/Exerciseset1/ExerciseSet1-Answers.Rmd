...
**Your Answers here:**
<br>1. In the dataset1.R file

<br>2. The cumulative returns of Apple during this period is `r round(100*sum(StockReturns[,2]),1)`%.
...
<br>3. Using the ticker YHOO and the same code in dataSet1.R, I duplicated it to dataSet2.R. The result is -

The cumulative returns of Yahoo during this period is `r round(100*sum(StockReturns_2[,3]),1)`%.
The average daily returns of Yahoo during this period is `r round(100*mean(StockReturns_2[,3]),3)`%;
The standard deviation of the daily returns of Yahoo during this period is `r round(100*sd(StockReturns_2[,3]),3)`%;
<br>
...
**Your Answers here:**
<br>1. Rows = nrow(t(StockReturns)) ncol = (t(StockReturns))
...
**Your Answers here:**
<br> 1. Yes, in the R script - dataSet1.R, in the code "startDate"
<br>
<br>2. The following is a portfolio of S&P, Apple, Yahoo, Goldman Sachs, and Microsoft
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
